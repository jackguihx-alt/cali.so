  ...
  if (!user) {
    return (
      <SignInButton mode="modal">
        ...
        >
          <UserArrowLeftIcon className="h-6 w-6 stroke-zinc-500 p-0.5 transition group-hover:stroke-zinc-700 dark:group-hover:stroke-zinc-200" />
        </button>
      </SignInButton>
    )
  }
  ...
  return (
    <SignOutButton>
      <button
        type="button"
        aria-label="退出登录"
        className="group rounded-full bg-gradient-to-b from-zinc-50/50 to-white/90 p-1 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur transition dark:from-zinc-900/50 dark:to-zinc-800/90 dark:ring-white/10 dark:hover:ring-white/20"
        title="点击退出登录"
      >
        <Image
          src={user.imageUrl}
